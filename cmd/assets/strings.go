...
func NewDownloader(cmdPath string) *steamDownloader {
	return &steamDownloader{cmdPath: cmdPath, outDir: "/tmp"}
}

func (d *steamDownloader) DownloadDepot(appID, depotID string, fileListPath ...string) (string, error) {
	var args []string

	if len(fileListPath) > 0 {
		args = append(args, "-filelist")
		args = append(args, fileListPath[0])
	}

	args = append(args, "-username")
	args = append(args, d.steamUsername)
	args = append(args, "-password")
	args = append(args, d.steamPassword)
	if d.steamAuthCode != "" {
		// args = append(args, "-filelist")
...
func ParseGameStrings(dl *steamDownloader, fileList string) error {
	path, err := dl.DownloadDepot(os.Getenv("DOWNLOADER_APP_ID"), os.Getenv("DOWNLOADER_DEPOT_ID"), fileList)
	if err != nil {
		return err
	}

	dirPath := filepath.Join(path, "/Data/Strings")
