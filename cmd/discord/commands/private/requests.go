		...
		builder.NewCommand("requests").
			...
			Handler(func(ctx common.Context) error {
				...
				case "view":
					...
					if requestID == "" && userID == "" {
						return ctx.Reply().IsError(common.UserError).Send("request id or user id needs to be provided")
					}
					...
					if err != nil {
						return ctx.Reply().IsError(common.ApplicationError).Send("failed to get requests", err.Error())
					}
				...
				case "set_status":
					requestID, _ := subOptions.Value("request_id").(string)
					if requestID == "" {
						return ctx.Reply().IsError(common.UserError).Send("review id needs to be provided")
					}

					status, ok := subOptions.Value("status").(string)
					if !ok {
						return ctx.Reply().IsError(common.UserError).Send("status cannot be left blank")
					}

					data, err := ctx.Core().Database().GetModerationRequest(ctx.Ctx(), requestID)
					if err != nil {
						return ctx.Reply().IsError(common.ApplicationError).Send("failed to get request", err.Error())
					}

					data.ActionStatus = models.ModerationStatus(status)
					data, err = ctx.Core().Database().UpdateModerationRequest(ctx.Ctx(), data)
					if err != nil {
						return ctx.Reply().IsError(common.ApplicationError).Send("failed to update request", err.Error())
					}
					d, _ := json.MarshalIndent(data, "", "  ")
