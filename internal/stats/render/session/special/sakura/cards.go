...
func generateCards(sessionData, careerData fetch.AccountStatsOverPeriod, cards session.Cards, _ []models.UserSubscription, opts common.Options) (*facepaint.Block, error) {
	...
	for _, column := range cards.Unrated.Overview.Blocks {
		for _, block := range column.Blocks {
			...
			case prepare.TagWN8:
				...
				maxWidthOverviewColumn[column.Flavor == session.BlockFlavorDefault] = max(maxWidthOverviewColumn[column.Flavor == session.BlockFlavorDefault], overviewSpecialIconSize)
			}
			maxWidthOverviewColumn[column.Flavor == session.BlockFlavorDefault] = max(maxWidthOverviewColumn[column.Flavor == session.BlockFlavorDefault], facepaint.MeasureString(block.Label, overviewStyle.blockLabel(block.Tag).Computed().Font).TotalWidth)
			maxWidthOverviewColumn[column.Flavor == session.BlockFlavorDefault] = max(maxWidthOverviewColumn[column.Flavor == session.BlockFlavorDefault], facepaint.MeasureString(block.Value().String(), overviewStyle.blockValue(block.Tag).Computed().Font).TotalWidth)
		}
	}
	...
	var innerFrameLeft = []*facepaint.Block{newPlayerNameCard(careerData.Account)}

	// unrated overview
	if shouldRenderUnratedOverview {
		if card := newOverviewCard(cards.Unrated.Overview, maxWidthOverviewColumn); card != nil {
			innerFrameLeft = append(innerFrameLeft, card)
		}
	}
	// // rating battles
	// if shouldRenderRatingOverview {
	...
	var innerFrames []*facepaint.Block
	innerFrames = append(innerFrames, facepaint.NewBlocksContent(styledInnerFrame, innerFrameLeft...))
	...
	content := facepaint.NewBlocksContent(styledContentFrame, innerFrames...)

	// resize and place background
	cardsFrameSize := content.Dimensions()
	background := imaging.Fill(backgroundImage, cardsFrameSize.Width, cardsFrameSize.Height, imaging.Center, imaging.Lanczos)
	content = facepaint.NewBlocksContent(style.NewStyle(),
		facepaint.MustNewImageContent(styledBackground, background), content,
	)

	var frame []*facepaint.Block
	frame = append(frame, content)
	frame = append(frame, newFooterCard(sessionData, cards, opts))

	return facepaint.NewBlocksContent(styledOuterFrame, frame...), nil
}
