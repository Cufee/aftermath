...
CREATE TABLE `new_user_connection` (`id` text NOT NULL, `created_at` datetime NOT NULL, `updated_at` datetime NOT NULL, `type` text NOT NULL, `verified` bool NOT NULL DEFAULT false, `reference_id` text NOT NULL, `permissions` text NULL DEFAULT '', `metadata` json NULL, `user_id` text NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `user_connection_user_id_fk` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE);
-- Copy rows from old table "user_connection" to new temporary table "new_user_connection"
INSERT INTO `new_user_connection` (`id`, `created_at`, `updated_at`, `type`, `reference_id`, `permissions`, `metadata`, `user_id`) SELECT `id`, `created_at`, `updated_at`, `type`, `reference_id`, `permissions`, `metadata`, `user_id` FROM `user_connection`;
