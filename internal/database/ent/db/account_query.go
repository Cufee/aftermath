...
// QueryClan chains the current query on the "clan" edge.
func (aq *AccountQuery) QueryClan() *ClanQuery {
	query := (&ClanClient{config: aq.config}).Query()
...
// QueryVehicleSnapshots chains the current query on the "vehicle_snapshots" edge.
func (aq *AccountQuery) QueryVehicleSnapshots() *VehicleSnapshotQuery {
	query := (&VehicleSnapshotClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithClan tells the query-builder to eager-load the nodes that are connected to
// the "clan" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithClan(opts ...func(*ClanQuery)) *AccountQuery {
	query := (&ClanClient{config: aq.config}).Query()
...
// WithVehicleSnapshots tells the query-builder to eager-load the nodes that are connected to
// the "vehicle_snapshots" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithVehicleSnapshots(opts ...func(*VehicleSnapshotQuery)) *AccountQuery {
	query := (&VehicleSnapshotClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withVehicleSnapshots = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withVehicleSnapshots; query != nil {
		if err := aq.loadVehicleSnapshots(ctx, query, nodes,
			func(n *Account) { n.Edges.VehicleSnapshots = []*VehicleSnapshot{} },
			func(n *Account, e *VehicleSnapshot) { n.Edges.VehicleSnapshots = append(n.Edges.VehicleSnapshots, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadClan(ctx context.Context, query *ClanQuery, nodes []*Account, init func(*Account), assign func(*Account, *Clan)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "clan_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AccountQuery) loadVehicleSnapshots(ctx context.Context, query *VehicleSnapshotQuery, nodes []*Account, init func(*Account), assign func(*Account, *VehicleSnapshot)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vehiclesnapshot.FieldAccountID)
	}
	query.Where(predicate.VehicleSnapshot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.VehicleSnapshotsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (aq *AccountQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withClan != nil {
			_spec.Node.AddColumnOnce(account.FieldClanID)
		}
	}
