...
func (sc *SessionCreate) SetPublicID(s string) *SessionCreate {
	sc.mutation.SetPublicID(s)
	return sc
}

// SetMetadata sets the "metadata" field.
func (sc *SessionCreate) SetMetadata(m map[string]string) *SessionCreate {
...
func (sc *SessionCreate) check() error {
	...
	if _, ok := sc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`db: missing required field "Session.updated_at"`)}
	}
	...
	if _, ok := sc.mutation.PublicID(); !ok {
		return &ValidationError{Name: "public_id", err: errors.New(`db: missing required field "Session.public_id"`)}
	}
	if v, ok := sc.mutation.PublicID(); ok {
		if err := session.PublicIDValidator(v); err != nil {
			return &ValidationError{Name: "public_id", err: fmt.Errorf(`db: validator failed for field "Session.public_id": %w`, err)}
		}
	}
	if _, ok := sc.mutation.Metadata(); !ok {
		return &ValidationError{Name: "metadata", err: errors.New(`db: missing required field "Session.metadata"`)}
	}
	if _, ok := sc.mutation.UserID(); !ok {
...
func (sc *SessionCreate) createSpec() (*Session, *sqlgraph.CreateSpec) {
	...
	if value, ok := sc.mutation.PublicID(); ok {
		_spec.SetField(session.FieldPublicID, field.TypeString, value)
