...
var iconsCache = make(map[string]*facepaint.Block, 6)
...
func GetRatingIcon(rating frame.Value, size float64) (*facepaint.Block, bool) {
	...
	block, err := facepaint.NewImageContent(style.Options(), img)
	if err != nil {
		return nil, false
	}

	iconsCache[name] = block
...
func NewRatingIcon(rating frame.Value) (*facepaint.Block, bool) {
	settings, ok := RatingIconSettings[GetRatingTierName(rating.Float())]
	if !ok {
		return facepaint.NewEmptyContent(style.NewStyle(style.SetWidth(1), style.SetHeight(1))), false
	}
	return RenderRatingIcon(settings)
}

func RenderRatingIcon(settings ratingIcon) (*facepaint.Block, bool) {
	...
	block, err := facepaint.NewImageContent(style.NewStyle(), ctx.Image())
	if err != nil {
		return nil, false
	}
	return block, true
